(function() {
    ...
    let userInterests = (localStorage.getItem('user_interests') || 'Naturaleza, Aventura, Trekking').trim();
    ...
    function populatePreview() {
        ...
        if (prevInterestPills) {
            ...
            let interestList = userInterests.split(',').map(i => i.trim()).filter(i => i !== '');
            if (interestList.length === 0) {
                interestList = ['Naturaleza', 'Aventura', 'Trekking'];
            }
            interestList.forEach(interest => {
                ...
                pill.className = 'badge rounded-pill bg-success text-white d-inline-flex align-items-center justify-content-center px-3 py-2 fw-semibold me-1 mb-1';
    ...
    if (btnSaveProfileInfo) {
        btnSaveProfileInfo.addEventListener('click', function(e) {
            ...
            if (!userInterests) {
                userInterests = 'Naturaleza, Aventura, Trekking';
            }
